...
4) Create a data representation for the experiment. `readMsExperiment()` takes
   the names of the data files (including the path to the directory where they
   are stored - configured below using the `MZML_PATH` variable) and a
   `data.frame` with the sample information for the respective data files as
   input. The resulting `MsExperiment` object (variable `std_data`) contains a
   reference to the MS data files and the sample information.
...
-   Use this information to determine rt width and m/z deviation. e.g. `ppm=` �
